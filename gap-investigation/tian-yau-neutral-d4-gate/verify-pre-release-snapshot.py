...
def wrong_end(v):
 ...
 for (pair,I,e,X,Y),coef in v.items():
  i,j=pair;p=c.terms[j][2]-c.terms[i][2];sgn=1 if p%2==0 else -1
  for aa,bb,poly in c.arrows:
   for target,sign in ([((i,bb),1)] if aa==j else [])+([((aa,j),sgn)] if bb==i else []):
    for exp,cc in poly:c.accum(out,(target,I,tuple(x+y for x,y in zip(e,exp)),X,Y),(-1)**len(I)*sign*cc*coef)
 return out
...
def calculate():
 ...
 for j,x in enumerate(src):
  ...
  etas=[]
  # Produce admissible, character-preserving exact perturbations outside
  # the harmonic representative support (hc of a normalized lift is zero).
  key=next(iter(vs[0]));pair,I,e,X,Y=key
  U=X if len(X)>1 else Y;offset=0 if len(X)>1 else 4
  assert len(U)>1
  ee=list(e);ii,jj=offset+U[0],offset+U[1]
  shift=3*max(1,(-ee[ii]+2)//3);ee[ii]+=shift;ee[jj]-=shift
  eta=c.hc({(pair,I,tuple(ee),X,Y):c.F(1)})
  assert eta and c.dv(eta)
  etas.append(eta)
  for eta in etas:
  ...
  badend=c.add(wrong_end(vs[0]),c.dv(vs[1]))
  assert badend,'wrong End commutator sign went undetected by first lift'
  counts['wrong_End_sign']=counts.get('wrong_End_sign',0)+1
 ...
 # Vertical-exact target representatives from the actual Koszul k=1 basis.
 for i in range(len(c.terms)):
  for j in range(len(c.terms)):
   if c.terms[j][2]-c.terms[i][2]!=2:continue
   bs=m.basis((i,j),0,1)
   for col in range(len(bs)):
    beta=c.lift(bs,s.eye(len(bs))[:,col]);bd=c.dv(beta)
    if bd:
     assert quotient(bd,span,nb)==s.zeros(target.cols,1)
     counts['target_vertical_boundary']+=1
