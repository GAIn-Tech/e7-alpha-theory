...
def main():
 ...
 for b in beta[2:]:
  key=next(k for k in b if len(k[3])==4 or len(k[4])==4);pair,I,e,X,Y=key
  U=X if len(X)==4 else Y;offset=0 if len(X)==4 else 4;ee=list(e);ii,jj=offset+U[0],offset+U[1]
