...
def compute():
 ...
 dh=2*( (B@h).T@(B@h))*0 # placeholder-free direction covector below
 ...
 for xi in (.1,1.,3.,100.):
  ...
  total=sum(channels.values());rows.append({'xi':xi,'channels':{k:pack(v) for k,v in channels.items()},'sum':pack(total),'omit_ghost':pack(total-gh),'omit_shift':pack(total-shift),'stationary_equation_residual':float(np.linalg.norm(H@delta+tad))})
 errors=[]
 for a,b in [(0,0),(.1,3),(4320,4320),(1000,1000)]:errors.append(float(abs(I(a,b)-HP.integral(a,b,adaptive=True))))
 return {'claim_boundary':'Action-derived Rxi diagram evaluation; cancellation status computed, not assumed; floating tensors and rounded mass shells, not a formal proof or full pole.','ward_differentiated_residual':ward,'master_adaptive_max_error':max(errors),'rows':rows,'cancellation_passed':all(abs(complex(z['sum']['real'],z['sum']['imag']))<1e-5 for z in rows),'source_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest()}
