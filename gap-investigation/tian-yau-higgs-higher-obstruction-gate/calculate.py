...
def main():
 ...
 # Full polynomial heavy/massless splitting: C0 -> C1 -> C2.
 zeros=[]
 for i in range(N):
  for j in range(N):
   p=c.terms[j][2]-c.terms[i][2]
   if p!=0:continue
   for _,I,e in m.basis((i,j),0,0):zeros.append(globalv((i,j),I,e))
 dkeys=[next(k for k in v if k[3]==k[4]==(0,)) for v in dom]
 D0=s.Matrix(len(dom),len(zeros),lambda i,j:s.Rational(c.total(zeros[j]).get(dkeys[i],0)))
 assert not any(M*D0)
 K=s.Matrix.hstack(*M.nullspace())
 boundaries=D0.columnspace();B=s.Matrix.hstack(*boundaries)
 reps=[];span=B
 for v in K.columnspace():
  if span.row_join(v).rank()>span.cols:reps.append(v);span=span.row_join(v)
 pivots=M.rref()[1]
 U=s.eye(len(dom))[:,list(pivots)]
 split=span.row_join(U)
 assert split.rows==split.cols and split.det()!=0
 projection=split.inv()[len(boundaries):len(boundaries)+len(reps),:]
 assert not any(projection*sol)
 assert len(reps)==12 and D0.rank()==18
 h2=U*(M*U).inv()
 assert M*h2==s.eye(M.rows) and -h2*b==sol
 polynomial={'C_dimensions':[D0.cols,M.cols,M.rows],'D0':a.sparse(D0),'D1':a.sparse(M),'D0_rank':D0.rank(),'D1_rank':M.rank(),'H_dimensions':[D0.cols-D0.rank(),len(reps),M.rows-M.rank()],'massless_representatives':a.sparse(s.Matrix.hstack(*reps)),'massless_projection_C1':a.sparse(projection),'heavy_lift_C2_to_C1':a.sparse(h2),'a2_massless_coordinates':a.sparse(projection*sol)}
 ...
 b4=s.Matrix([s.Rational(S4.get(k,0)) for k in keys]);sol4=-h2*b4
 a4=c.add(*[c.scale(v,c.F(str(sol4[i]))) for i,v in enumerate(dom)])
 assert c.total(a4)==c.scale(S4,-1) and not any(projection*sol4)
 S5=bracket(a1,a4,1,1);assert not S5
 S6=bracket(a2,a4,1,1)
 # Only a finite-order statement is certified. S6 is saved as the next source.
 print('order4 primitive terms',len(a4),'order5 source',len(S5),'order6 source',len(S6),flush=True)
 ...
 # Distinguish exact-representative freedom from genuine massless second-order shifts.
 # All positive-monad-degree polynomial closed shifts annihilate a1 in the bracket.
 for col in range(K.cols):
  z=c.add(*[c.scale(v,c.F(str(K[i,col]))) for i,v in enumerate(dom)])
  assert not c.total(z) and not bracket(a1,z,1,1)
 import serre_pairing as sp
 allc=[u.unpack(x) for x in mod['charged_basis']]
 ind=s.zeros(18,4)
 for j,r in enumerate(d4['records']):
  alpha=u.unpack(r['surviving_total_cocycle']);delta=bracket(a1,alpha,1,1)
  assert not c.total(delta)
  for slot in range(2):
   for i,v in enumerate(allc):
    probe=remap(v,N,N+slot)
    scalar=u.action(delta,probe)
    scalar={key:value for key,value in scalar.items() if key[0]==(N+slot,N+slot)}
    scalar=remap(scalar,N+slot,N)
    value=sp.residue(scalar)
    assert value==s.Rational(ser['tensor'][j][i][slot])
    ind[slot*9+i,j]=value
 assert ind.rank()==4
 ...
 out={'passed':True,'embedding':roots,'a1':c.pack(a1),'quadratic_source':c.pack(S2),'quadratic_matrix':a.sparse(M),'quadratic_rhs':a.sparse(-b),'quadratic_solution':a.sparse(sol),'domain_generators':[compact(v) for v in dom],'target_keys':[[list(t) for t in k] for k in keys],'quadratic_matrix_rank':M.rank(),'a2':c.pack(a2),'a2_compact':compact(a2),'cubic_source':c.pack(S3),'quartic_source_a3_zero':c.pack(S4),'polynomial_splitting':polynomial,'a4':c.pack(a4),'a4_compact':compact(a4),'order5_source':c.pack(S5),'order6_source':c.pack(S6),'massless_survivor_indeterminacy':a.sparse(ind),'massless_survivor_indeterminacy_rank':ind.rank(),'polynomial_closed_lift_changes_checked':K.cols,'controls':controls,'source_hashes':{str(f.relative_to(P.parent)).replace('\\','/'):sha(f) for f in used},'claim_boundary':'Exact charged A4 sub-DGLA in E8, actual quadratic primitive and next MC source; not full higher superpotential, stabilized vacuum or all-orders family.'}
